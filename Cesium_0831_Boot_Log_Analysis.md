...
- 04:24:37–04:24:49 — **camera probe: every sensor candidate fails** (`bf2557` id reads 0x0/0xffffffff, `sp5508`, `gc05a3` — all `IMGSENSOR_PROBE miss` on every index). Expected on MP16-Cesium-V, which has no camera by design — but the BSP still spends ~12 s probing; the sensor list/camera HAL should be stripped for this SKU.
...
|---|---|---|---|---|
| 1 | 🔴 | **Unpersisted wall clock** — the MT6359P PMIC RTC exists but the system has **no RTC backup battery by design**, so time is lost on every power-off. Software must compensate: persist last-known time to /data at shutdown/periodically and restore it at boot (AOSP `PersistentNetworkTimeStore` / time-detector config, or MTK power-off-clock support), so the device boots with approximately-correct, monotonically *forward* time instead of the build timestamp. Current behavior: 16.6-year jump + 1.8 s backwards step every cold boot; breaks TLS/scheduling/log forensics pre-NTP | dmesg 04:24:33 RTC=1970/2010; logcat line 940 `PersistentNetworkTimeStore … no reliable RTC`; `Time changed in by 525846107 seconds` | **CVTE** (software time-persistence; no hardware change expected) |
| 2 | 🟠 | **Camera-less SKU not reflected in BSP** — MP16-Cesium-V has **no camera by design**, yet the kernel sensor table still lists 3 candidates (bf2557 ×2 addresses, sp5508, gc05a3) and spends **~12 s of boot time** (04:24:37→04:24:49) power-cycling and probing them, spamming ~30 error lines per boot. Fix: strip the sensor list / camera HAL for this SKU and remove `android.hardware.camera.*` feature flags so apps and CTS see "no camera" gracefully rather than a broken one | dmesg `IMGSENSOR_PROBE miss` for every candidate, ids 0x0/0xffffffff | **CVTE** (per-SKU device config) |
...
1. **Clock chain:** the PMIC RTC has no backup battery (by design), so time is lost at power-off → kernel falls back to MTK 2010 epoch → AOSP overrides with **build time** ("no reliable RTC") → 16.6-year jump + 1.8 s backwards step every cold boot. This explains all cross-log timestamp anomalies in the whole 0831 capture set. Fix is **software**: persist last-known-good time to /data and restore at early boot (and ensure restored time only moves forward), plus prompt NTP once connected. **CVTE.**
...
3. **SKU/BSP config gaps:** MP16-Cesium-V is camera-less by design but the BSP still probes 3 sensors (~12 s boot cost + error spam) and declares camera hardware; WiFi cfg/TX-power tables missing; DT resources invalid. Classic PVT integration cleanup. **CVTE.**
