...
31. To CVTE: run Shane's method (Section 2.3: flip bytes inside the preloader RSA signature, write to boot0 only with the unit already fused) and report two things: whether SP Flash Tool refuses the write, which is the "intercepted during flashing" claim of conflict 12, and whether the same recovery procedure works on a unit that was fused first and corrupted afterwards.
