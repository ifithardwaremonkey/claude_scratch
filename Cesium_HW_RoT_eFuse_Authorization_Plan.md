...
| Revision | 1.19 draft, 18 September 2026. Full history in Section 10 |
...
**r1.17 amendment (dates revised r1.18): lot 1 may fuse if confidence reaches Medium by 2 October.** The MP build date has moved to 15 October (holiday on 2 October), which puts it inside the window where a bounded fused lot can be authorized, and CVTE states that removing the closed configuration now would itself delay MP. The posture becomes Option C in Section 3.1: a Go/No-Go on 2 October, the original M4 gate date, decides whether lot 1 fuses, against the Medium criteria of Section 3.5 plus an OTA freeze on fused serials until the rollout controls J1 to J4 are demonstrated. A Go is recorded as "Go, subject to S7": the 200-cycle randomized power-loss soak on the test units in Logan, which starts on their arrival about 25 September and completes about 5 October, must end with zero unrecoverable units before the line fuses on 15 October. Any unrecoverable unit converts the Go to a No-Go. S7 is the one test that adds measurable brick-risk evidence after the M4 gate, so it is attached as a confirmation rather than used as a reason to slip the gate. If Medium is not reached, lot 1 ships unfused on a profile CVTE prepares now as the fallback. Either way MP does not slip again. Full authorization for all production and for Malata remains at High.
...
1. Revive bricked unit J26080143-0A00076 via forced BROM download (E5). If this cannot be shown, closed configuration is not accepted at all. **Status 15 Sep: attempted per CVTE's instruction (hold the download key during power-on) and failed; no enumeration in 120 seconds.** **Status 17 Sep (Source J): CVTE reproduced the brick on its own fused unit (preloader signed with a key that does not match the burned hash) and recovered it over BROM USB with a three-condition procedure: SP Flash Tool in "Format all + download" and already listening, the Force Flash button held low before 12 V is applied and through power-on, and firmware signed to the burned key. E5 is demonstrated at CVTE. Shane repeats the exact procedure on J26080143-0A00076 (S3b in the test plan); that repeat closes E5 and X3.** This gates fused production (X3), not the cohort shipment.
...
| **J. CVTE (Simon Huang), 17 September 2026** | Deliberate brick and recovery on a fresh CVTE unit: all firmware re-signed with a new key and flashed (boots), then eFuse burned with the original hash (BROM rejects the preloader, unit bricked). Recovery over BROM USB with SP Flash Tool in "Format all + download", Download clicked before power, Force Flash button held low before 12 V and through power-on, firmware signed to the burned key. Screenshot of the successful flash; photo of the Force Flash button on the PCBA under the back cover. Statements: "the 8371 does indeed have a USB BROM"; eFuse burn is the first line step; mis-signed firmware is "intercepted during the flashing process" once fused. Request to Shane for a method to break the signature after fusing | First demonstrated BROM recovery of a fused MT8371 with a failed preloader on this program. Closes the CVTE half of E5 and S3. The procedure differs from the 14 Sep instruction in button timing and tool state (Section 3.4). The interception claim is restated, not tested, by this experiment (conflict 12). Tool log, serial and versions requested (ask 29) |
| **K. CVTE (Simon Huang), 18 September 2026, 01:43** | Reply to the burn log. States Shane's steps match CVTE's ("flash a preloader with a different hash first, then flash the eFuse"). SP Flash Tool V6 screenshot: fused unit, Firmware Upgrade scene, preloader VCOM, package `VKC1_20260916_other_hash`, error "download preloader data failed. write protected partition[preloader_a] from host file failed". States that once fused a device "cannot be tampered with after it leaves the factory". Asks whether removing the eFuse step from the line is still necessary; with William Chen recommends fusing directly as mature; says removing it means production-documentation changes that could affect schedule | Misreads the order of events in Source I (Shane fused first, corrupted afterwards; CVTE the reverse). The screenshot is the first evidence that a pre-flash check exists on the download path: a **wrong-key** preloader is refused on a fused unit. Shane's **corrupted-signature, correct-key** preloader was accepted (Source I). Conflict 12 is narrowed accordingly. The production question is answered by Option C: decision on 2 October, fallback prepared in parallel; CVTE has still not quantified the documentation cost (assumption 29) |
...
| 12 | **Pre-flash signature check (r1.13).** CVTE, Aug 2026: three-stage enforcement including a preloader-level check of the firmware signature against the eFuse key **before flashing**. Source I: SP Flash Tool wrote a preloader with a corrupted RSA signature to boot0 of a fused unit and reported success. **CVTE restated it 17 Sep (Source J): after the eFuse is burned, "attempting to flash incorrectly signed firmware will be directly intercepted during the flashing process."** | The pre-flash check does not exist on the download path used (DAA off, unsigned DA accepted, host writes whatever it is given). The download path is a depot hazard on fused units: a wrong or mis-signed preloader is a hard brick. Mitigations: depot profile excludes boot0 unless a chip-level fix requires it (H1, R19); depot artifacts hash-verified against the release manifest before use (H4); ask CVTE which stage, if any, performs the check they described (ask 25). **Still open after Source J.** CVTE's own brick was made by burning the fuse after flashing mismatched firmware, which bypasses any pre-flash check by construction, so their statement is untested by their own experiment. Shane's method (flash a mis-signed preloader onto an already fused unit) is the direct test, and CVTE has asked for it. Conflict resolves when CVTE runs Shane's method and reports whether the tool refuses the write. **Narrowed 18 Sep (Source K).** CVTE's screenshot shows the tool refusing to write a preloader from a package signed with a different key onto a fused unit: "write protected partition [preloader_a]". Shane's write of the production preloader with 32 corrupted signature bytes was accepted (Source I). Both observations fit one mechanism: the pre-flash check compares the public-key hash carried in the preloader certificate against the eFuse hash. A wrong-key image fails it; a corrupted-signature image under the correct key passes it and is then rejected by BROM at boot. Consequence: the depot is protected against the wrong-image error but not against a corrupted correct-key image, which is what hash verification of every depot artifact before use (H4) is for; the boot0 exclusion in the depot profile stands. Two facts still needed from CVTE (ask 31 revised): whether the refusal is enforced by the DA or by the host tool, and whether it applies in "Download only" as well as "Firmware Upgrade". CVTE's statement that a fused device "cannot be tampered with" is true for running unauthorized code and false for bricking: Source I shows a corrupted preloader can be written and bricks the unit. |
...
| Behavior on preloader corruption or mismatch | BROM download mode | BROM download mode; re-flash preloader, no auth file | **BROM halts; no spontaneous USB.** Forced-download entry (E5) is the only path, and CVTE has demonstrated it (Source J): Force Flash button held before power, tool listening in "Format all + download", correctly signed firmware |
...
**What it proves.** The fuse works end to end, and a fused MT8371 whose preloader fails authentication does not offer USB on its own. That is the one failure mode Section 2.2 says is new with fusing, demonstrated cleanly. **r1.15: it no longer proves the unit is unrecoverable.** CVTE reproduced the same BROM-level authentication failure on 17 September (a validly signed preloader under the wrong key, which BROM rejects the same way it rejects a corrupted signature) and recovered the unit over BROM USB. The difference between the two attempts is procedural, and Section 3.4 lists it. Until Shane repeats CVTE's procedure on J26080143-0A00076, the first article is still bricked, but the mode is now classed as depot-recoverable pending that repeat rather than as a hard brick.
...
|---|---|---|---|
| MP lot 1 | Software RoT, plus 25–50 unit fused pilot ring built by the MP-intent line process and placed at ICON-controlled sites | All units fused | **Decided on 2 Oct (M4 gate), subject to S7 by about 5 Oct.** Fused if confidence is Medium (Section 3.5) on that date and S7 ends with zero unrecoverable; otherwise unfused on the fallback profile CVTE prepares now |
...
| 22 to 24 Sep | G3 and G11 signed, keys off Basecamp | iFIT |
| 22 to 26 Sep | L1 to L8 and I1 to I6 witnessed at CVTE on video with logs (Basecamp item 12 to Simon); J1 to J3 demonstrated on a bad build in a lab ring; Malata ME1 to ME4 start | Simon, iFIT release engineering, Malata |
| about 25 Sep | Ring and test units arrive in Logan. ICON S5 repeat; **S7 starts the same day**; US depot rehearsal (H1) | Shane |
| **2 Oct** | **Go/No-Go on fusing lot 1 (M4 gate): Medium reached, OTA freeze in place, or ship it unfused on the fallback profile. A Go is "Go, subject to S7"** | Allen, sign-off per Section 3.5 |
| about 5 Oct | S7 complete. Zero unrecoverable confirms the Go; any unrecoverable unit converts it to No-Go and lot 1 ships unfused | Shane, Allen |
| 15 Oct | MP build (internal date, not shared with Malata) | CVTE |
...
The critical path to 2 October is iFIT's, not CVTE's: G3 signed and J1 to J3 demonstrated, both in the week of 22 September. Every hardware test Medium requires is complete by about 20 September; what follows is paperwork and a line witness, not data. The one question to settle this week is whether the OTA backend already does staged rings with an automatic halt. If it has to be built, the 2 October answer is No-Go regardless of the tests, and that should be decided now rather than discovered at the gate.
...
### 3.4 Cohort release status and simplified remaining test set (15 September, updated 17 September)

**Status 17 September (Source J).** CVTE has demonstrated BROM recovery of a fused unit whose preloader fails authentication. Details and the differences from ICON's failed attempt are in "Interpreting the failure" below. The active decision branch moves from "S3 fail, ROM command disable unset" to "S3 pass at CVTE, ICON repeat pending". Confidence stays **Low** until Shane's repeat on the bricked first article, then **eligible for Medium**.
...
|---|---|---|
| 1–3 Forced download | "Press and hold the Volume Up (Download) key during startup." Photo shows an on-board tact switch near the 12 V input on the C.G520.702 board; reaching it means opening the enclosure. No video, no tool log, no BROM versus preloader distinction. **17 Sep (Source J): CVTE bricked a fresh unit by fusing it after flashing firmware signed to a different key, then recovered it. Procedure: (1) SP Flash Tool in "Format all + download", Download clicked so the tool is listening, firmware signed to the burned key loaded; (2) Force Flash button held before 12 V and kept low through power-on; (3) tool recognizes the device and flashes. Screenshot of the successful flash supplied. CVTE confirms "the 8371 does indeed have a USB BROM"** | ICON attempted the 14 Sep instruction on the deliberately bricked unit: **no USB enumeration in 120 s**. **The CVTE-side half of P2 and E5 is now demonstrated.** ICON repeats CVTE's exact procedure on J26080143-0A00076 (S3b). See "Interpreting the failure" below |
...
| Recovery: download key held during power-on, 120 s USB monitoring | **No MediaTek device enumerated** | E5, P2 **fail as executed**; superseded by CVTE's 17 Sep procedure, repeat pending (S3b) |
...
3. **Whether the procedure works on a healthy fused unit** (S3). This is the discriminator the plan has asked for since revision 1.2. A healthy unit that enumerates in BROM or preloader mode proves the path exists on this fuse map; a healthy unit that does not proves the map or the procedure is wrong for all 100. **Overtaken 17 Sep: CVTE demonstrated it on a bricked fused unit, which is the stronger result.**

**What CVTE's 17 September recovery shows, and why ICON's attempt failed (r1.15).** CVTE's brick and Shane's brick are the same BootROM event. In both, BROM loads the preloader from boot0, computes the RSA verification against the burned public-key hash, and rejects it. Whether the signature is corrupted (Shane) or valid under a different key (CVTE) does not matter to BROM; both are an authentication failure at the first stage, before any preloader code runs. So CVTE's recovery is evidence for Shane's unit. Three differences between the two procedures account for the outcome, and all three are now conditions in S3 and S3b:

| Condition | CVTE 17 Sep (recovered) | ICON 15 Sep (no enumeration) | Why it matters |
|---|---|---|---|
| Button timing | Held **before** 12 V is applied and kept low **through** power-on | "Held during power-on"; the log records the key held for the power-on, not whether it was already low when the rail came up | BROM samples the download pin in the first milliseconds after reset. A pin that goes low after sampling is ignored; there is no second chance until the next power cycle |
| Tool state | "Format all + download", Download clicked, tool actively polling **before** power | Tool armed in Download, then later attempts in different modes | The BROM USB window is short (a few seconds) and BROM only stays in download mode if the host completes the handshake inside it. A tool that is not polling misses the window, and the bus monitor may show nothing because BROM never completes enumeration without a host driver claim |
| Firmware signature | Matches the burned key | Both signed images tried, but the flash never started | Irrelevant to enumeration; relevant to whether the flash then succeeds, because BROM verifies the DA and the preloader written to boot0 against the burned hash |

The first two are the likely cause of the 15 September failure. The recording in Source I ("no enumeration in 120 s") is consistent with a BROM window that opened and closed unclaimed, as much as with a BROM that never opened. S3b instructs Shane to repeat with the button already low when 12 V is applied and the tool already polling in "Format all + download", and to log the tool console rather than only the bus monitor. Two residuals remain open until that repeat: CVTE's unit was fused with mismatched firmware already on it, Shane's was fused first and corrupted afterwards, so any BROM state difference from the order of events is untested; and the "Format all" step erases userdata, so the depot procedure H1 is a full wipe, which changes nothing for a bricked unit but must be recorded.

**Consequence for the plan.** The one failure mode Section 2 classes as new with fusing (preloader authentication failure) has a demonstrated depot recovery on this fuse map. Section 2.3's "hard brick" becomes "depot-recoverable with enclosure open, pending ICON repeat". The Section 6.3 delta for boot0 failures moves toward the zero-delta case that section already describes. The Force Flash button is under the back cover (CVTE: "accessible only to technician"), which settles assumption 11: soft brick at the depot, hard brick for the customer, exactly as an unfused unit with a corrupt preloader would be.
...
| S3 | **Done at CVTE 17 Sep on a bricked fused unit (stronger than the healthy-unit case).** ICON repeats on its own healthy fused unit if it has one, with CVTE's three conditions | CVTE (done), ICON | CVTE: screenshot supplied; tool log and unit serial still requested (ask 29). ICON: tool console log, VID:PID log |
| S3b (new) | ICON, on the bricked first article J26080143-0A00076, CVTE's exact procedure | Logan | Tool console log through "Download Ok", boot to Android, read-efuse unchanged. Closes E5 and X3 |
| S4 | CVTE, same unit, straight after S3 | CVTE | SP Flash Tool console log, before/after `read-efuse` logs |
| S5 | **CVTE first (r1.16), on five cohort units now**, with UART logs on the two interruption units and video of the power cuts; ICON repeats on the ring units when they arrive as the independent check | CVTE, then Logan | CVTE: update_engine log and UART console per unit, before/after fingerprint, video of the two cuts. ICON: own records. Customer release may proceed on CVTE's S5 pass; the ICON repeat feeds X2 and confidence, not the release |
| S7 | ICON, on the test units after they arrive. Not a cohort-release item | Logan | Own records. Programmable supply and UART logging; about one week unattended |
| H1 (US depot) | ICON, on the first returned or sacrificial ring unit, at the US service site | US depot | Recovery log with the Force Flash procedure and the depot profile. Must be proven in the US regardless of CVTE's result |
...
|---|---|
| The ring and test units are an **exception shipment**. Nothing flows to Logan by default | CVTE pulls 15 to 20 ring units plus the 7 test units out of the lot and ships them directly to Logan, on 0814, not reflashed. ICON arranges the shipment and provides consoles or 12 V bench fixtures at ICON sites, since bare tablets are not the shipped product. Transit about one week. **r1.16: S5 no longer waits on this shipment; CVTE runs it first (below). What the shipment is for:** the ring units are field exposure at iFIT sites and the first ring for every future cohort OTA, which cannot happen at CVTE by definition; the test units are for S7, the US depot rehearsal (H1) and the later destructive cases, all of which ICON runs itself |
| **S5 at CVTE first (r1.16).** The five S5 units, the OTA server access, the 12 V supplies and the hands are all at CVTE today, and the shipment would otherwise hold the customer release to about 25 September | CVTE runs S5 on five cohort units now, with UART console on the two interruption units, `update_engine` logs per serial and video of the two power cuts. ICON accepts a CVTE pass as the S5 result for the customer-release decision. ICON repeats S5 on five ring units on arrival as the independent check; that repeat feeds X2 and the confidence level, not the release. Trade: about a week gained on the customer release against independence on the first S5 data point, which the ICON repeat restores |
...
| S3 | Forced download on a **healthy** cohort unit: enclosure open, SP Flash Tool V6 in "Format all + download" with Download clicked and the BROM VCOM driver installed, Force Flash button held low **before** 12 V and through power-on; log tool console and USB VID/PID seen (MediaTek BROM `0E8D:0003`, preloader `0E8D:2000`) | Enumerates and the tool connects; **do not flash** (cancel at the handshake, since "Format all" would erase the unit). **Demonstrated at CVTE 17 Sep on a bricked unit** | 1 h | P2 (first half), conflict 11 |
| S3b (new) | Same procedure on bricked first article J26080143-0A00076, with signed 0909 loaded, and let the flash run | "Download Ok"; boots 0909; read-efuse unchanged | 1 h | E5, X3, P2 (second half) |
...
| S5 | Apply the 0814→0909 OTA to five cohort units. On two of them cut 12 V during payload write and during first boot of the new slot. **CVTE runs it first with logs and video; ICON repeats on the ring units** | All five on 0909; the two interrupted units fall back or resume; no brick | 1 day | X1 first transition; D2/D4 in cohort scope; P6 |
...
- **S1, S2, S4, S5, S6 and the paper items pass, S3 passes** → release the ICON-controlled ring immediately; release the customer units from CVTE's dock after S5 has passed on the ring and the customer units have taken the 0814→0909 OTA at CVTE with logs. Fused-production work continues in parallel on the full plan. **This is the active branch from 17 Sep: S3 has passed at CVTE.** The customer release no longer waits on MediaTek's ask 15; S3b (ICON repeat on the bricked unit) gates confidence Medium and fused production, not the cohort.
...
| **Low** | Fuse mechanism proven; recovery not proven | S1, S2, S4, S6 pass; S5 zero unrecoverable; S3 **failed** or not run; E5 not demonstrated | Cohort to ICON ring only. Production ships unfused. This was the level on 15 September. **On 17 September the level is still Low, but the recovery leg is now demonstrated by CVTE (S3 pass); the remaining Medium items are S3b on the bricked unit, S1/S2/S4/S5/S6 execution, A0-b and the Group G gates** |
...
| Result | Effect |
|---|---|
| S3 pass | Low → eligible for Medium once E5 also passes on the bricked unit. **S3 passed at CVTE 17 Sep (Source J)** |
| S3b pass (ICON repeat on J26080143-0A00076) | Closes E5 and X3. With S1, S2, S4, S5, S6, A0-b and G3/G4/G7/G11, confidence becomes Medium |
| S3b fail with CVTE's exact procedure | Stays Low. The difference between the two units (fused-then-corrupted versus mismatched-then-fused) becomes the question for MediaTek (ask 15 rewritten) |
...
| E5 | STOP-SHIP BROM recovery | J26080143-0A00076 and one freshly bricked fused unit recover via forced download and boot a signed build. **Half closed 17 Sep: CVTE's freshly bricked fused unit recovered (Source J). J26080143-0A00076 pending S3b** | Test lead, CVTE |
...
Malata has no fuse-station history on this program, though ICON reports Malata has burned eFuses on other MediaTek programs (assumption 18, r1.16). Tier 2 is a transfer, not a re-derivation: the board, the fuse image and the BootROM behavior are the same as CVTE's, so nothing about Cesium's fuse behavior is re-proven on Malata hardware. What is proven is Malata's station, interlock, depot and custody.

**Malata early start (r1.16), before CVTE's questions are closed.** Recommended now, limited to sacrificial units:

| Item | Content | Why now |
|---|---|---|
| ME1 | Malata receives `efuse_iFitG520.img` (hash-verified by ICON against G4) and the signed `DA_BR.bin` by the H4 controlled channel: **iFIT Arena PLM (r1.19)**, as released controlled items with SHA-256 recorded in the item, pulled by Malata under its existing supplier access, not project chat or email | The custody route is itself a Tier 2 requirement; exercising it early finds the gaps |
| ME2 | Malata fuses **three to five sacrificial units** with that image and reads back: `sbc_en`, hash, DAA, SLA, JTAG-disable. ICON compares to the CVTE read-back | First-article read-back for Tier 2, three weeks early. Hash identical to CVTE's or stop |
| ME3 | Malata bricks one of them by Shane's method (Source I) and recovers it with CVTE's Force Flash procedure (Source J), on Malata's own bench, tool build and fixture. Tool console log and video | A second ODM reproducing the recovery on a different setup is the best evidence the procedure transfers. Also tests conflict 12 (does Malata's tool refuse the mis-signed write) on a second tool installation |
| ME4 | Malata confirms its unfused factory-test profile (I6): logs eFuse state per serial, does not require `sbc_en`, version-controlled and not operator-selectable | Malata's MP units ship unfused from 2 October like CVTE's; this is needed regardless of the fuse decision |
| Not asked | S5, S7, or any fusing beyond the three to five sacrificial units | S5 and S7 answer platform questions CVTE already has the units and OTA access for; every fused unit is permanent |

Cost: three to five permanently fused sacrificial units and an earlier controlled transfer of the fuse image and DA. Benefit: the fusing decision no longer rests on one vendor's reading of one experiment, and Tier 2 starts about three weeks earlier.
...
| Decision | Malata fusing authorized only after CVTE has shipped at least one fused lot with no fuse-attributable field return. ME1 to ME4 above may run before that; they do not authorize anything |
...
Until Tier 3 passes, fused units in the field receive no OTA. This is the control that makes Option B tolerable and it costs nothing under Option A. **Under Option C (r1.17) it is a condition of the 2 October Go: J1 to J4 demonstrated before lot 1 ships, or the fused lot receives no OTA until they are.** The cohort is exempt once released, because its units are on 0909 and its first field OTA is the same MP release the control group takes, under whatever rings exist then.
...
- Signed DA custody documented and distributed only through controlled channels (H4). **r1.19: the H4 channel is iFIT Arena PLM.** Controlled items: `efuse_iFitG520.img`, `DA_BR.bin`, the signed depot firmware package, and the pinned SP Flash Tool, driver and DA versions (L7), each with its SHA-256 in the item record and the fuse-image-to-firmware pairing (L5) as an item relationship. Released to CVTE and Malata under supplier access after S1 verifies the hash; downloads are logged. **Never in Arena:** any private key (root, image, DA, Android platform; these are G3, HSM or escrow with named holders) and the signed secure-boot-disable preloader (G6, inventoried, not distributed). Confirm Arena supplier permissions can restrict these items to CVTE and Malata.
...
11. Whether the forced-download button is reachable on the assembled console without disassembly. This defines hard versus soft brick for the product. **Closed 17 Sep: CVTE's photo shows the Force Flash button on the PCBA under the back cover, "accessible only to technician". Soft brick at the depot, hard brick for the customer.**
...
18. Malata's readiness: the 100-unit trial on 30 July was a CVTE run. Malata has not fused a unit on this program. **Corrected r1.16: ICON reports Malata is familiar with eFuse burning from other MediaTek programs.** That lowers the line-side risk (R13 for Malata) but does not remove Tier 2, because the station script, fuse image, interlock and factory-test gate are Cesium-specific. Early-start items ME1 to ME4 are in Tier 2.
...
29. **(Added r1.17) What CVTE must change on the line to ship unfused**, and how many days it costs. CVTE claims a delay; the plan requires the unfused profile to exist before 2 October regardless.
...
**New asks (17 Sep, r1.15), sent with Shane's brick method:**

29. To CVTE: the SP Flash Tool console log from the 17 Sep recovery (BROM handshake through "Download Ok"), the unit serial, the tool and DA versions, and whether the DA used was `DA_BR.bin` from the 0909 package. The screenshot shows success; the log shows which stage enumerated and how long the BROM window stayed open.
30. To CVTE: is "Format all + download" required for BROM entry, or only chosen for the demonstration? If "Download only" or "Firmware upgrade" also works from BROM, the depot can recover a unit without erasing userdata.
31. To CVTE: run Shane's method (Section 2.3: flip bytes inside the preloader RSA signature, write to boot0 only with the unit already fused) and report two things. **Revised 18 Sep after Source K: CVTE's 18 Sep test used a wrong-key package, not Shane's corrupted-signature production preloader, so the direct test is still outstanding. Also requested: which component enforces the "write protected partition" refusal (DA or host tool), and whether it applies in "Download only" as well as "Firmware Upgrade".** **Method delivered 17 Sep: ICON sent Simon the full G520 eFuse Burn Log (Source I) as "additional background, procedure Shane used to brick tablet".** The two things: whether SP Flash Tool refuses the write, which is the "intercepted during flashing" claim of conflict 12, and whether the same recovery procedure works on a unit that was fused first and corrupted afterwards.
32. To CVTE: confirm the Force Flash button is the BROM download pin (`KPCOL0` or equivalent) sampled by BootROM at reset, and not a preloader-polled key. The recovery result implies BROM-level, but the 14 Sep instruction named "Volume Up", which the preloader polls.

**Answered or overtaken by Source J:** ask 15 (BROM enters USB download mode on a fused MT8371 with a failed preloader, given the button is held before power and the host is polling); ask 16 (the effective instruction is BROM-level); ask 24 (the eMMC test point is no longer needed as the primary path; keep as a fallback if S3b fails). Assumption 11 closed.
...
**Changes in revision 1.19 (18 September).** Source K (CVTE, 18 Sep 01:43) ingested. CVTE misreads the burn log's order of events (Shane fused first, corrupted afterwards) and supplies a screenshot of SP Flash Tool refusing a wrong-key preloader on a fused unit ("write protected partition [preloader_a]"). Conflict 12 narrowed: the pre-flash check appears to compare the certificate's key hash to the eFuse, which blocks a wrong-key image and passes a corrupted-signature correct-key image; depot boot0 exclusion and artifact hash verification (H4) stand. Ask 31 revised to name the still-outstanding direct test and two questions (DA or tool; Download only or Firmware Upgrade). CVTE asks to keep fusing on the line and drop the fallback; answered by Option C, decision 2 October, fallback in parallel, documentation cost still unquantified (assumption 29). H4 channel named: iFIT Arena PLM for integrity-controlled artifacts with hashes and supplier access; private keys and the permissive preloader excluded. ME1 updated to match. **The consolidated Basecamp post to CVTE, iFIT and Malata was sent on 18 September** with Decision 1 (cohort release, target 26 Sep), Decision 2 (fusing Go/No-Go 2 Oct, subject to S7, lot 1 explained, fallback in parallel, no build date stated), Simon items 1 to 12 including the corrections to Source K, Shane items 1 to 5, Malata items 1 to 7 with Arena named as the artifact channel, and the timeline through early October. It is the action record; the test plan v1.5 cross-reference maps its items to test cases.

**Changes in revision 1.18 (17 September, night).** Decision date moved from 10 October back to 2 October, the original M4 gate date, at iFIT's direction: management commitment to the gate is kept, and the eight days between 2 and 10 October contained no test that measures brick risk, only paperwork and the line witness. S7, the 200-cycle randomized power-loss soak, is the one measurable brick-risk item that completes after the gate, so a Go is recorded as "Go, subject to S7" and confirmed or converted around 5 October, ten days before the line runs. Calendar in Section 3.2 rewritten with owners; the week of 22 September now carries G3, G11, the CVTE line witness on video (Basecamp item 12) and the J1 to J3 demonstration. The 15 October build date is internal to iFIT and CVTE and is not shared with Malata, whose production follows CVTE's. Test plan v1.5 issued alongside.
...
**Changes in revision 1.15 (17 September).** Source J, CVTE (Simon Huang) 17 September, ingested. CVTE took a fresh unfused unit, signed all firmware with a new key, flashed it (boots), then burned the eFuse with the original hash, producing a preloader that BROM rejects. CVTE recovered it over BROM USB: SP Flash Tool in "Format all + download" with Download clicked before power, the Force Flash button held low before 12 V and through power-on, firmware signed to the burned key. CVTE confirms the MT8371 has a USB BROM. Effect: E5 is demonstrated at CVTE (half closed), S3 is passed at CVTE on a bricked unit, the active decision branch moves to "S3 pass" and customer release no longer waits on MediaTek. Section 3.4 gains a comparison of CVTE's procedure with ICON's failed 15 Sep attempt; the button timing (already low when 12 V is applied) and the tool state (polling before power) are the two probable causes of the failure. New step S3b: Shane repeats CVTE's exact procedure on J26080143-0A00076; a pass closes E5 and X3 and makes confidence eligible for Medium. Section 2.3 reclassifies the preloader-authentication brick from "hard brick" to "depot-recoverable pending ICON repeat". Conflict 12 stays open: CVTE restated the pre-flash interception claim but its own experiment could not test it; CVTE has asked for Shane's method, which is the direct test (ask 31). Asks 29 to 32 added; asks 15, 16 and 24 overtaken; assumption 11 closed (button under the back cover, technician only). Shane's brick method sent to CVTE the same day as the full Source I PDF, so ask 31 is delivered and awaits CVTE's run. Confidence: **Low**, recovery leg now demonstrated.
