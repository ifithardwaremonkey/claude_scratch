...
| 19 | **Re-test only if re-signed** | 0909 is the MP release candidate, so no separate final-build re-test is scheduled. If G1 (separate release-signing step) or a key decision (K0) produces a re-signed 0909, re-run G2, G4, N3, B1, B2, C3, C5, D2, D4, F1, M5 on that artefact; reserve 3 days for it. Any re-sign changes hashes; nothing tested on the compile-signed 0909 transfers automatically | Final artefact pairing signed off (L5) |
...
| 4 `input.xml`, `GFH_CONFIG.ini` | Files attached (2.75 KB, 751 B) | ICON has them. Read `Disable_Rom_Cmd` / `brom_magic_cmd_mode_permanent_dis`, `Enable_DAA`, `jtag_en`, `debug_en` (S1). Shane's read-back already shows `daa_en`, `sla_en`, `jtag_dis` off |
...
| 9 Depot artefacts | `DA_BR.bin` and signed 0909 in VKC1_20260909.zip; SP Flash Tool Selector v1.2444 and USB driver attached; use V6 | Artefacts in hand. Written depot procedure with a full-image profile still missing (H1, R19). S4 exercises the artefacts |
...
1. **Which stage was corrupted.** "The bootloader" is either the preloader or LK. MediaTek's download-key path is polled by the preloader; if the preloader itself was corrupted, that path never runs and BROM entry depends on BROM's own USB handshake window or on BROM command mode being enabled. If LK was corrupted, the intact preloader should still enumerate as a preloader USB device for one to two seconds at every power-on, and silence there points at the procedure or the port, not the fuse.
...
**Simplified remaining set for cohort release.** Everything below is ICON-executable with artefacts in hand. Estimated two working days.
...
| G2 | STOP-SHIP Certificate chain | `otacert`, `platform.x509.pem` and the AVB image key all resolve to the ICON certificate; none matches an AOSP test key. **Closed on 0909** (SHA-256 `61:7C:CC:...:1B:BF`, O = Icon Health and Fitness). Repeat on any re-signed artefact | ICON |
...
| F1 | Must be run on the exact 0909 artefact that ships, and repeated if it is re-signed | Fleet escape hatch must be proven on the shipping artefact |
...
**Answers received 14 Sep (Source H)**: items 4, 5, 8, 9, 10 answered with artefacts; item 6 half-answered (`MTK_SEC_USBDL` only); item 7 answered in principle; items 1 to 3 answered as an instruction, not a demonstration, and the instruction failed at ICON; item 11 is a schedule objection, answered in Section 3.4; item 12 deferred to the dev-keys analysis. Status per item is in Section 3.4.
